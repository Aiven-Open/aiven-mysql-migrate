...
class MySQLMigrationToolBase(ABC):
    ...
    def execute_migration(self, migration_method: MySQLMigrateMethod) -> Optional[str]:
        ...
        LOGGER.debug("Executing dump command: %s", " ".join(dump_cmd))
        LOGGER.debug("Executing import command: %s", " ".join(import_cmd))
...
class MyDumperTool(MySQLMigrationToolBase):
    ...
    def _create_temp_cnf_file(self, connection_info: MySQLConnectionInfo) -> Path:
        """Create temporary .cnf file with credentials for secure password handling."""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.cnf', delete=False) as temp_cnf:
    ...
    def get_dump_command(self, migration_method: MySQLMigrateMethod) -> List[str]:
        ...
        if not self.temp_cnf_file:
            self.temp_cnf_file = self._create_temp_cnf_file(self.source)
        ...
        cmd = [
            ...
            "--replica-data",
            "--source-data",
            f"--outputdir={self.temp_dir.name}"
        ]
        if self.source.ssl:
            cmd += ["--ssl-mode=REQUIRED"]
        if self.databases:
    ...
    def get_import_command(self, migration_method: Optional[MySQLMigrateMethod] = None) -> List[str]:
        ...
        if not self.temp_target_cnf_file:
            self.temp_target_cnf_file = self._create_temp_cnf_file(self.target)
        if not self.temp_dir:
            self.temp_dir = self._create_temp_directory()
        ...
        cmd = [
            ...
            "--threads=0",
            f"--directory={self.temp_dir.name}",
    ...
    def cleanup(self) -> None:
        ...
        if self.temp_cnf_file and self.temp_cnf_file.exists():
            try:
                self.temp_cnf_file.unlink()
                self.temp_cnf_file = None
            except (OSError, IOError) as e:
                LOGGER.warning("Failed to cleanup temporary .cnf file: %s", e)

        if self.temp_target_cnf_file and self.temp_target_cnf_file.exists():
            try:
                self.temp_target_cnf_file.unlink()
                self.temp_target_cnf_file = None
            except (OSError, IOError) as e:
                LOGGER.warning("Failed to cleanup temporary target .cnf file: %s", e)
